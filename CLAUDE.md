...
4. Commit sample JSON files to `client/public/samples/` — `flag.json`, `pflag.json`, `pflag2.json`
5. Add a root `.gitignore` (see design.md section 17 for contents — critical: `server/pb_data/` must be gitignored before first commit)

**Weirdo must not regenerate the atlas or modify the sample files.** Both are Jaime's artifacts. The samples are ground truth — if a sample doesn't render correctly, fix the interpreter, not the sample.
...
11. Verify using the sample files in `client/public/samples/` — fetch them via URL (`/samples/flag.json` etc.), do not hardcode:
    - `flag.json` — single thread, minecraft: block names, turtle animates, red/blue/yellow/green wool renders with Faithful textures
    - `pflag.json` — 4 threads, all start simultaneously, each draws a different color stripe in parallel
    - `pflag2.json` — 4 threads with hex colors; the blue stripe (`#0000ff80`) must render **semi-transparent** (50% opacity), visibly different from the opaque stripes. If it renders solid blue, alpha parsing is broken.
    
    Add a "Load Sample" dropdown to the UI that fetches these files by URL — useful for testing and for students/faculty to see example programs without uploading anything.
...
- **Sample JSON files in `client/public/samples/` are ground truth.** If a sample doesn't render correctly, the interpreter is wrong. Never modify the samples to match the interpreter.
- **Samples are browser-accessible at `/samples/flag.json` etc.** Vite serves `public/` as static files. The UI must include a "Load Sample" dropdown that fetches these by URL.
